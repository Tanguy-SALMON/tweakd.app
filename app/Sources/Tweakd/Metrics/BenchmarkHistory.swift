...
//  ~/Library/Application Support/\(Brand.name)/benchmark-history.json, so the trail
...
enum BenchmarkHistoryStore {
    ...
    nonisolated static var url: URL {
        URL(fileURLWithPath: NSHomeDirectory())
            .appendingPathComponent("Library/Application Support/\(Brand.name)/benchmark-history.json")
    }
