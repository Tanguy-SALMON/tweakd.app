...
/// A well-known process tweakd offers a one-tap priority preset for.
...
final class PriorityManager: ObservableObject {
    ...
    private func plistPath(_ id: String) -> String {
        "\(launchAgentDir)/\(Brand.priorityLabel(id)).plist"
    }
    ...
    ///
    /// Kernel threads and tweakd itself are filtered out — you can't renice
    ...
    nonisolated static func resetAllOffMain(patterns: [String], launchAgentDir: String) -> CommandResult {
        ...
        for item in items where Brand.priorityPrefixes.contains(where: item.hasPrefix) && item.hasSuffix(".plist") {
    ...
    nonisolated static func installLaunchAgent(path: String, id: String, pattern: String, nice: Int) -> Bool {
        let label = Brand.priorityLabel(id)
    ...
    /// Shell lines that reset every known target to nice 0 and unload tweakd's
    ...
    nonisolated static func revertScriptLines() -> [String] {
        ...
        lines.append("# Remove \(Brand.name) priority LaunchAgents (both current and pre-rename names):")
        for prefix in Brand.priorityPrefixes {
            lines.append("for f in \"$HOME/Library/LaunchAgents/\"\(prefix)*.plist; do [ -f \"$f\" ] && { launchctl unload \"$f\" 2>/dev/null; rm -f \"$f\"; }; done; true")
        }
        return lines
    }
    ...
    func recountManaged() {
        let items = (try? FileManager.default.contentsOfDirectory(atPath: launchAgentDir)) ?? []
        managedCount = items.filter { i in Brand.priorityPrefixes.contains(where: i.hasPrefix) && i.hasSuffix(".plist") }.count
    }
}
