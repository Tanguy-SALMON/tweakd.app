...
enum TweakCatalog {
    ...
    static let all: [Tweak] = [
        ...
        Tweak(
            ...
            revertCommand: "for p in \"$HOME/Library/Application Support/Firefox/Profiles/\"*/; do f=\"$p/user.js\"; if [ -f \"$f\" ] && /usr/bin/grep -qE 'tweakd privacy|MacTweak privacy' \"$f\"; then /bin/rm -f \"$f\"; fi; done; true",
            statusCommand: "/bin/cat \"$HOME/Library/Application Support/Firefox/Profiles/\"*/user.js 2>/dev/null | /usr/bin/grep -qE 'tweakd privacy|MacTweak privacy' && echo APPLIED",
            appliedWhenOutputContains: "APPLIED",
            tags: [.privacyFocused], recommended: false
        ),
        ...
        Tweak(
            ...
            summary: "Downloads the curated StevenBlack ad/tracker/malware domain list and routes those domains to 0.0.0.0 in /etc/hosts, so ads never load — system-wide, across every browser and app. Speeds up pages by skipping ad requests. The list auto-refreshes weekly in the background. Your existing /etc/hosts entries are preserved; only a clearly-marked tweakd block is added or removed. Needs a network connection to apply.",
            category: .security, privilege: .admin, risk: .moderate, sipRequired: false,
            applyCommand: AdBlock.rebuildCommand,
            // Both of these accept the pre-rename `# MacTweak-adblock-*` markers as
            // well. A Mac that turned ad-blocking on before the rename has ~200k
            // entries fenced by the old markers: matching only the new ones would
            // report the tweak as OFF while it was very much on, and revert would
            // leave every one of those entries behind with no way to remove them.
            revertCommand: AdBlock.removeCommand,
            statusCommand: "\(AdBlock.anyMarkerGrep) /etc/hosts && echo ON || echo OFF",
            appliedWhenOutputContains: "ON",
            tags: [.security, .privacyFocused], recommended: false
        ),
        Tweak(
